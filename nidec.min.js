...
class Paperless {
  ...
  async getValue(table_name, key, value) {
    const response = await fetch(
      this.base_url +
        this.service +
        `?table_name=${table_name}&query_key=${key}&query_value=${value}`
    );
    const data_response = await response.json();
    if (data_response) {
      return data_response;
...
class IO extends Service {
  ...
  uploadFile(file) {
    ...
    return this.postQuery("upload/", {}, formData);
  }
...
class Production extends Service {
  ...
  getCounterUpdateInput(number){
    return this.get(`production/?database=counter_update_input&query_key=mc&query_value=${number}`)
  }

  getCounterUpdate(number){
    return this.get(`production/?database=counter_update&query_key=mc&query_value=${number}`)
  }

  getCounterConfig(number){
    return this.get(`production/?database=counter_config_input&query_key=mc&query_value=${number}`)
  }
